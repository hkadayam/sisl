...
btree_status_t Btree< K, V >::create_root_node(bnodeid_t& out_root_node) {
    ...
    out_root_node = root->get_node_id();
...
template < typename K, typename V >
btree_status_t Btree< K, V >::read_and_lock_root(bnodeid_t id, BtreeNodePtr& node_ptr, locktype_t int_lock_type,
...
template < typename K, typename V >
btree_status_t Btree< K, V >::read_and_lock_child(bnodeid_t child_id, BtreeNodePtr& child_node,
                                                  const BtreeNodePtr& parent_node, uint32_t parent_ind,
...
/* It read the node, take the lock and recover it if required */
template < typename K, typename V >
btree_status_t Btree< K, V >::read_and_lock_sibling(bnodeid_t id, BtreeNodePtr& node_ptr, locktype_t int_lock_type,
                                                    locktype_t leaf_lock_type, void* context) const {
...
btree_status_t Btree< K, V >::get_child_and_lock_node(const BtreeNodePtr& node, uint32_t index,
                                                      BtreeNodeInfo& child_info, BtreeNodePtr& child_node,
                                                      locktype_t int_lock_type, locktype_t leaf_lock_type,
                                                      void* context) const {
    if (index == node->get_total_entries()) {
        ...
        child_info = node->get(index, false /* copy */);
    }
...
template < typename K, typename V >
btree_status_t Btree< K, V >::write_node_sync(const BtreeNodePtr& node, void* context) {
    return (write_node_sync(node, context));
}
...
template < typename K, typename V >
btree_status_t Btree< K, V >::write_node(const BtreeNodePtr& node, const BtreeNodePtr& dependent_node, void* context) {
...
void Btree< K, V >::read_node_or_fail(bnodeid_t id, BtreeNodePtr& node) const {
    BT_REL_ASSERT_CMP(read_node(id, node), ==, btree_status_t::success, );
}
 ...
 */
template < typename K, typename V >
btree_status_t Btree< K, V >::upgrade_node(const BtreeNodePtr& my_node, BtreeNodePtr child_node, void* context,
    ...
    BT_NODE_DBG_ASSERT_EQ(my_node->m_common_header.is_lock, 1, my_node);
done:
    return ret;
}

template < typename K, typename V >
btree_status_t Btree< K, V >::_lock_and_refresh_node(const BtreeNodePtr& node, locktype_t type, void* context,
...
template < typename K, typename V >
btree_status_t Btree< K, V >::_lock_node_upgrade(const BtreeNodePtr& node, void* context, const char* fname, int line) {
...
#ifndef NDEBUG
    if (type == locktype_t::WRITE) { node->m_common_header.is_lock = 0; }
#endif
    node->unlock(type);
...
BtreeNodePtr Btree< K, V >::alloc_leaf_node() {
    ...
    BtreeNodePtr n = alloc_node(true /* is_leaf */, is_new_allocation);
    if (n) {
...
BtreeNodePtr Btree< K, V >::alloc_interior_node() {
    bool is_new_allocation;
    BtreeNodePtr n = alloc_node(false /* is_leaf */, is_new_allocation);
    if (n) {
...
template < typename K, typename V >
BtreeNode< K, V >* Btree< K, V >::init_node(uint8_t* node_buf, bnodeid_t id, bool init_buf, bool is_leaf) {
    BtreeNode< K, V >* ret_node{nullptr};
    btree_node_type node_type = is_leaf ? m_cfg.leaf_node_type() : m_cfg.interior_node_type();

    switch (node_type) {
    case btree_node_type::VAR_OBJECT:
        ret_node = new VarObjSizeNode< K, V >(node_buf, id, init_buf, is_leaf);
        break;

    case btree_node_type::FIXED:
        ret_node = new FixedObjSizeNode< K, V >(node_buf, id, init_buf, is_leaf);
        break;

    case btree_node_type::VAR_VALUE:
        ret_node = new VarValueSizeNode< K, V >(node_buf, id, init_buf, is_leaf);
        break;

    case btree_node_type::VAR_VALUE:
        ret_node = new VarKeySizeNode< K, V >(node_buf, id, init_buf, is_leaf);
        break;
...
template < typename K, typename V >
void Btree< K, V >::free_node(const BtreeNodePtr& node, void* context) {
        ...
        if (info.node == node.get()) {
            *out_info = info;
            ...
            LOGTRACEMOD(btree, "REMOVING node {} from {} locked nodes list, its size = {}", (void*)info.node,
                        (ltype == LOCKTYPE_WRITE) ? "write" : "read", pnode_infos->size());
            return true;
...
void Btree< K, V >::check_lock_debug() {
    ...
    for (const auto& x : wr_locked_nodes) {
        x.dump();
    }
    for (const auto& x : rd_locked_nodes) {
        x.dump();
    }
