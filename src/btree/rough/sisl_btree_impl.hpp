    ...
    retry:
        ...
        if (child_node->is_merge_needed(m_bt_cfg)) {
            ...
            uint32_t node_end_indx =
                my_node->has_valid_edge() ? my_node->total_entries() : my_node->total_entries() - 1;
...
#ifndef NDEBUG
        if (ind != my_node->total_entries() && child_node->total_entries()) { // not edge
        ...
        if (ind > 0 && child_node->total_entries()) { // not first child
...
private:
        ...
        while (curr_ind <= end_ind) { // iterate all matched childrens
            ...
            if (is_range_put_req(put_req) && child_node->is_leaf()) {
                THIS_BT_LOG(DEBUG, btree_structures, my_node, "Subrange:s:{},e:{},c:{},nid:{},edgeid:{},sk:{},ek:{}",
                            start_ind, end_ind, curr_ind, my_node->node_id(), my_node->edge_id(),
                            subrange.get_start_key()->to_string(), subrange.get_end_key()->to_string());
            }

#ifndef NDEBUG
            K ckey, pkey;
            if (curr_ind != int_cast(my_node->total_entries())) { // not edge
    ...
    void get_leaf_nodes(bnodeid_t bnodeid, std::vector< BtreeNodePtr< K > >& leaves) const {
        ...
        HS_ASSERT_CMP(DEBUG, node->total_entries(), >, 0);
        if (node->total_entries() > 0) {
            BtreeLinkInfo p = node->get(0, false);
    ...
    btree_status_t split_node_replay(btree_journal_entry* jentry, const btree_cp_ptr& bcp) {
        ...
        if (jentry->is_root) {
            ...
            THIS_BT_LOG(INFO, btree_generics, ,
                        ...
                        child_node1->node_id(), parent_node->node_id(), m_cfg.get_name());
        ...
        THIS_BT_LOG(INFO, btree_generics, ,
                    ...
                    child_node1->node_gen(), m_cfg.get_name());
        if (jentry->is_root) {
