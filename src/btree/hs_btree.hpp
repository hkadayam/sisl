...
class HSBtree : public Btree< K, V > {
    ...
    btree_status_t free_btree(const BtreeNodePtr& start_node, blkid_list_ptr free_blkid_list, bool in_mem,
        ...
        const auto ret =
            post_order_traversal(start_node, [this, free_blkid_list, in_mem, &free_node_cnt](const BtreeNodePtr& node) {
     ...
     */
    template < typename K, typename V >
    bool Btree< K, V >::verify_node(bnodeid_t bnodeid, BtreeNodePtr parent_node, uint32_t indx, bool update_debug_bm) {
    ...
    btree_status_t create_btree_replay(btree_journal_entry* jentry, const btree_cp_ptr& bcp) {
        ...
        BtreeNodePtr root = reserve_leaf_node(BlkId(m_root_node_id));
