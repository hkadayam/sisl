...
btree_status_t Btree< K, V >::post_order_traversal(locktype_t acq_lock, const auto& cb) {
    BtreeNodePtr root;
    btree_status_t ret = read_and_lock_root(m_root_node_id, root, acq_lock, acq_lock, nullptr);
...
template < typename K, typename V >
btree_status_t Btree< K, V >::post_order_traversal(const BtreeNodePtr& node, locktype_t acq_lock, const auto& cb) {
    uint32_t i{0};
    ...
    if (!node->is_leaf()) {
        ...
        while (i <= node->get_total_entries()) {
            ...
            BtreeNodePtr child;
            ret = read_and_lock_child(child_info.bnode_id(), child, node, i, acq_lock, acq_lock);
    ...
    if (ret == btree_status_t::success) { cb(node); }
    return ret;
}

template < typename K, typename V >
void Btree< K, V >::get_all_kvs(std::vector< pair< K, V > >& kvs) const {
    post_order_traversal(locktype_t::READ, [this, &kvs](const auto& node) { node->get_all_kvs(kvs); });
}
...
void Btree< K, V >::to_string(bnodeid_t bnodeid, std::string& buf) const {
    ...
    unlock_node(node, acq_lock);
                ...
                btree_status_t merge_node_replay(btree_journal_entry* jentry, const btree_cp_ptr& bcp) {
                    BtreeNodePtr parent_node = (jentry->is_root) ? read_node(m_root_node_id) : read_node(jentry->parent_node.node_id);
...
template < typename K, typename V >
void Btree< K, V >::validate_sanity_child(const BtreeNodePtr& parent_node, uint32_t ind) {
    BtreeNodeInfo child_info;
    K child_first_key;
    K child_last_key;
    K parent_key;
...
template < typename K, typename V >
void Btree< K, V >::validate_sanity_next_child(const BtreeNodePtr& parent_node, uint32_t ind) {
    BtreeNodeInfo child_info;
    K child_key;
    K parent_key;
...
template < typename K, typename V >
void Btree< K, V >::print_node(const bnodeid_t& bnodeid) {
    std::string buf;
    BtreeNodePtr node;
