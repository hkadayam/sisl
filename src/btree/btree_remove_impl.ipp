...
btree_status_t Btree< K, V >::do_remove(const BtreeNodePtr& my_node, locktype_t curlock, BtreeRemoveRequest& rreq) {
...
#endif
        bool is_found = my_node->remove_one(rreq.m_keys, rreq.m_outkey, rreq.m_outval);
#ifndef NDEBUG
        my_node->validate_key_order();
#endif
        if (is_found) {
...
retry:
    ...
    auto [found, ind] = my_node->find(rreq.m_keys, nullptr, nullptr);
    ...
    if (child_node->is_merge_needed(m_cfg)) {
...
btree_status_t Btree< K, V >::check_collapse_root(void* context) {
    ...
    static thread_local std::vector< BtreeNodePtr > s_nodes;
    s_nodes.clear();
...
template < typename K, typename V >
btree_status_t Btree< K, V >::merge_nodes(const BtreeNodePtr& parent_node, uint32_t start_indx, uint32_t end_indx,
                                          void* context) {
    ...
    for (auto indx = start_indx; indx <= end_indx; ++indx) {
        ...
        if (indx == start_indx) {
            balanced_size = m_cfg.ideal_fill_size();
