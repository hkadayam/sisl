...
#define BT_LOG(level, msg, ...)                                                                                        \
    { LOG##level##MOD_FMT(btree, (_BT_LOG_METHOD_IMPL(, m_cfg, )), msg, ##__VA_ARGS__); }

#define BT_NODE_LOG(level, msg, node, ...)                                                                             \
    { LOG##level##MOD_FMT(btree, (_BT_LOG_METHOD_IMPL(, m_cfg, node)), msg, ##__VA_ARGS__); }
...
#define BT_ASSERT(assert_type, cond, ...)                                                                              \
    { assert_type##_ASSERT_FMT(cond, _BT_LOG_METHOD_IMPL(, m_cfg, ), msg, ##__VA_ARGS__); }

#define BT_ASSERT_CMP(assert_type, val1, cmp, val2, ...)                                                               \
    { assert_type##_ASSERT_CMP(val1, cmp, val2, _BT_LOG_METHOD_IMPL(, m_cfg, ), msg, ##__VA_ARGS__); }
...
#define BT_NODE_ASSERT(assert_type, cond, node, ...)                                                                   \
    { assert_type##_ASSERT_FMT(cond, _BT_LOG_METHOD_IMPL(, m_cfg, node), msg, ##__VA_ARGS__); }

#define BT_NODE_ASSERT_CMP(assert_type, val1, cmp, val2, node, ...)                                                    \
    { assert_type##_ASSERT_CMP(val1, cmp, val2, _BT_LOG_METHOD_IMPL(, m_cfg, node), msg, ##__VA_ARGS__); }
...
#define ASSERT_IS_VALID_INTERIOR_CHILD_INDX(is_found, found_idx, node)                                                 \
    BT_NODE_DDB_ASSERT((!is_found || (found_idx < (int)node->get_total_entries()) || node->has_valid_edge()), node,    \
